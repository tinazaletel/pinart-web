...
export default async function PogodbePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.workspace}><header className={styles.topbar}><div><p className={styles.eyebrow}>POGODBE</p><h1>Dogovor, brez ugibanja.</h1></div><StoparicaBliznjica /></header>{sme ? <ContractWorkspace base={base} /> : <Zaklenjeno funkcija="contracts" base={base} />}</section>
  </main>;
}
