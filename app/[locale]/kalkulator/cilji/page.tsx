...
export default async function CiljiPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className={styles.shell}><DashboardSidebar base={base} active="goals" /><section className={styles.workspace}><header className={styles.topbar}><div><p className={styles.eyebrow}>{locale === 'en' ? 'BUSINESS GOALS' : 'POSLOVNI CILJI'}</p><h1>{locale === 'en' ? 'A number with a reason.' : 'Številka z razlogom.'}</h1></div></header>{sme ? <GoalsWorkspace base={base} /> : <Zaklenjeno funkcija="businessInsights" base={base} jeEn={locale === 'en'} />}</section></main>;
}
