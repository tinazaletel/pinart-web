...
.${styles.projectsPage} > *,
.${styles.projectsLayout} > *{min-width:0;}
.${styles.projectsToolbar} > label{min-width:0;}
.${styles.projectsToolbar} input{width:100%;min-width:0;box-sizing:border-box;}
.${styles.projectIndex} > button span strong,
.${styles.projectIndex} > button span small,
.${styles.projectStory} h2,
.${styles.projectStory} > header span{overflow-wrap:anywhere;}
.pw-nazaj{display:none;align-items:center;gap:.4rem;margin:0 0 .8rem;padding:.55rem .95rem;border:1px solid var(--line);border-radius:999px;background:oklch(98% .008 87 / .92);font:700 .62rem var(--font-sans),sans-serif;color:var(--ink);cursor:pointer;}
...
.${styles.projectIndex} > button{grid-template-columns:auto minmax(0,1fr) auto;}
/* Detajl projekta = svoja stran (view-swap), ne "pod seznamom". Vidnost
   krmili pw-mobilni-pogled na .projectsLayout, ne selectedId (ta ostane
   nedotaknjen, da namizje še vedno privzeto kaže prvi projekt). */
.${styles.projectsLayout}[data-pw-pogled="seznam"] .${styles.projectStory}{display:none;}
.${styles.projectsLayout}[data-pw-pogled="detajl"] .${styles.projectIndex}{display:none;}
.${styles.projectsLayout}[data-pw-pogled="detajl"] .pw-nazaj{display:inline-flex;}
}
`;
...
export default function ProjectsWorkspace({ base, zunanjiFilter, iskanje, onIskanje, status, onStatus, datumOd: datumOdZunaj, datumDo: datumDoZunaj, onDatumOd, onDatumDo }: Props) {
  ...
  /* Detajl projekta se izriše v .projectStory panelu. Na mobilnem (≤980px, >640px) je ta panel
     POD seznamom, zato klik navidezno »ne naredi nič« — ob izbiri ga zdrsnemo v pogled.
     Na ≤640px gre korak dlje: detajl postane SVOJA stran (view-swap, glej overflowFix zgoraj),
     krmiljena z ločenim mobilniPogled stanjem — selectedId ostane nedotaknjen, da namizje
     še vedno privzeto kaže prvi projekt (fallback || visible[0] zgoraj). */
  const storyRef = useRef<HTMLElement>(null);
  const [mobilniPogled, setMobilniPogled] = useState<'seznam' | 'detajl'>('seznam');
  const selectProject = (id: string) => { setSelectedId(id); setMobilniPogled('detajl'); if (typeof window !== 'undefined' && window.matchMedia('(max-width: 980px)').matches) requestAnimationFrame(() => storyRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })); };

  return <div className={styles.projectsPage}><style dangerouslySetInnerHTML={{ __html: overflowFix }} />
    {!zunanjiFilter && <ArhivFilter
      iskanje={search}
      onIskanje={setSearch}
