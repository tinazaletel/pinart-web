...
export default function SwotAgent({ base = '', onSave }: SwotAgentProps) {
  ...
  const napisi = async () => {
    ...
    catch { setNapaka(L('Pupa trenutno ni dosegljiva — lahko izpolniš brez nje.', 'Pupa is currently unavailable — you can fill it in yourself.')); setPot('rocno'); setIzid(v => v || { prednosti: '', slabosti: '', priloznosti: '', nevarnosti: '' }); } finally { setDela(false); }
  };
