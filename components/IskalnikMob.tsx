...
/* Enoten iskalnik: na namizju polni iskalni pas, na mobilnem (≤640px) IKONA v krogu,
   ki se ob kliku razširi v celo-širinski input (kot Arhiv / Komunikacija). En vzorec povsod.
   Postavi ga v flex vrstico (dobi flex:1). Starš naj bo position:relative, da se overlay
   razširi čez celotno vrstico. */
export default function IskalnikMob({ vrednost, naVrednost, placeholder, label }: {
  ...
  return (
    <div className="iskmob">
      <label className="iskmob-poln">
        <MagnifyingGlass size={18} weight="bold" aria-hidden />
        <input value={vrednost} onChange={e => naVrednost(e.target.value)} placeholder={placeholder} aria-label={label || placeholder} />
      </label>
      ...
      <style>{`
        .iskmob { position: relative; flex: 1 1 auto; min-width: 0; display: flex; }
        .iskmob-poln { flex: 1 1 auto; min-width: 0; display: flex; align-items: center; gap: .75rem; min-height: 3rem; padding: 0 1rem; border: 1px solid var(--line, rgba(17,17,17,.12)); border-radius: 999px; background: color-mix(in oklch, var(--paper, #fff) 92%, transparent); color: color-mix(in oklch, var(--ink, #111) 55%, transparent); }
        .iskmob-poln input { width: 100%; border: 0; outline: 0; background: transparent; font: 500 .9rem var(--font-sans), system-ui, sans-serif; color: var(--ink, #111); }
        .iskmob-krog { display: none; }
        .iskmob-over { display: none; }
        @media (max-width: 640px) {
          .iskmob { flex: none; position: static; }
          .iskmob-poln { display: none; }
          .iskmob-krog { display: inline-flex; align-items: center; justify-content: center; flex: none; width: 3rem; height: 3rem; border-radius: 50%; border: 1px solid var(--line, rgba(17,17,17,.12)); background: color-mix(in oklch, var(--paper, #fff) 92%, transparent); color: var(--ink, #111); cursor: pointer; }
