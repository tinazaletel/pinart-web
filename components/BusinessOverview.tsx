...
const STATUS: Record<OfferStatus, string> = {
  draft: 'Osnutek', sent: 'Čaka odgovor', accepted: 'Sprejeta', rejected: 'Zavrnjena',
};
...
export default function BusinessOverview({ base }: { base: string }) {
  ...
  const statusTone = (status: string) => status === 'Čaka odgovor' || status === 'Odprt' || status === 'V pregledu' ? 'waiting' : status === 'Sprejeta' || status === 'Podpisana' || status === 'Plačan' || status === 'Aktivna' ? 'success' : status === 'Zavrnjena' ? 'danger' : 'neutral';
  const statusOptions = (type: string) => type === 'Ponudba' ? ['Osnutek', 'Čaka odgovor', 'Sprejeta', 'Zavrnjena'] : type === 'Pogodba' ? ['Prejeta', 'V pregledu', 'Osnutek', 'Aktivna', 'Podpisana'] : type === 'Račun' ? ['Odprt', 'Plačan'] : [];
  const updateDocumentStatus = (type: string, id: string, label: string) => {
    if (type === 'Ponudba') {
      const value = ({ Osnutek: 'draft', 'Čaka odgovor': 'sent', Sprejeta: 'accepted', Zavrnjena: 'rejected' } as Record<string, OfferStatus>)[label];
