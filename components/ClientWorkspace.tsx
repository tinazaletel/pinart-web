...
import { MagnifyingGlass, Globe, Phone, EnvelopeSimple } from '@phosphor-icons/react';
...
export default function ClientWorkspace() {
  ...
  const [dnDatum, setDnDatum] = useState('');
  ...
  const addDnevnik = () => {
    ...
    const vnos: DnevnikVnos = { id: crypto.randomUUID(), clientId: selected.id, projectId: dnProjekt || undefined, tip: dnTip, datum: dnDatum || new Date().toISOString().slice(0, 10), besedilo, created: new Date().toISOString() };
  ...
  return <div className={styles.clientPage}>
    ...
    <div className={styles.clientLayout}><section className={styles.clientDirectory}><header><div><p className={styles.eyebrow}>IMENIK</p><h2>{visible.length} strank</h2></div></header>{visible.length ? visible.map(client => { const result = stats.find(item => item.id === client.id); return <button key={client.id} className={selected?.id === client.id ? styles.clientActive : ''} onClick={() => setSelected(client)}><span className={styles.clientInitials}>{client.name.split(/\s+/).map(word => word[0]).join('').slice(0, 2).toUpperCase()}</span><span><strong>{client.name}</strong><small>{client.contact || client.email || 'Brez kontakta'}</small></span><span><strong>{money(result?.revenue || 0)}</strong><small>plačano</small></span><i>›</i></button>; }) : <p className={styles.clientEmpty}>Ni najdenih strank.</p>}</section>
      <section className={styles.clientProfile}>{selected ? <><header><span className={styles.clientProfileAvatar}>{selected.name.split(/\s+/).map(word => word[0]).join('').slice(0, 2).toUpperCase()}</span><div><p className={styles.eyebrow}>PROFIL STRANKE</p><h2>{selected.name}</h2><span>{selected.contact || 'Brez kontaktne osebe'}</span></div><button onClick={() => { setEditing(selected); setOpen(true); }}>Uredi</button></header><div className={styles.clientContacts}><span className={styles.clientSplet}><small>Spletna stran</small>{selected.website ? <a href={selected.website.startsWith('http') ? selected.website : `https://${selected.website}`} target="_blank" rel="noopener noreferrer"><Globe size={15} weight="bold" /><strong>{selected.website.replace(/^https?:\/\//, '').replace(/\/$/, '')}</strong></a> : <strong>—</strong>}</span><span><small>E-pošta</small><strong>{selected.email || '—'}</strong></span><span><small>Telefon</small><strong>{selected.phone || '—'}</strong></span><span><small>Davčna št.</small><strong>{selected.tax || '—'}</strong></span><span><small>Naslov</small><strong>{selected.address || '—'}</strong></span></div><div className={styles.clientFinance}><span><small>Plačano</small><strong>{money(selectedInvoices.filter(item => item.paid).reduce((sum, item) => sum + item.amount, 0))}</strong></span><span><small>Odprti računi</small><strong>{money(selectedInvoices.filter(item => !item.paid).reduce((sum, item) => sum + item.amount, 0))}</strong></span><span><small>Stroški</small><strong>{money(selectedExpenses.reduce((sum, item) => sum + item.amount, 0))}</strong></span></div><div className={styles.clientLinki}><div className={styles.clientLinkGlava}><div><h3>Kontakti</h3><small className={styles.clientLinkPodnaslov}>Vsaka oseba pri stranki — svoj telefon in mail. Klic ali mail se sam zabeleži spodaj v dnevniku.</small></div>{!samoOgled && <button type="button" className={styles.clientLinkGumb} onClick={() => { setKontaktUrejam(null); setKontaktOdprt(p => !p); }}>{kontaktOdprt ? 'Prekliči' : '+ Dodaj osebo'}</button>}</div>{!samoOgled && kontaktOdprt && <div className={styles.clientEditor}><div><p className={styles.eyebrow}>{kontaktUrejam ? 'UREDI OSEBO' : 'NOVA OSEBA'}</p><h2>Kontaktna oseba.</h2></div><form onSubmit={shraniKontakt}><label>Ime in priimek<input required name="ime" defaultValue={kontaktUrejam?.ime} /></label><label>Vloga<input name="vloga" defaultValue={kontaktUrejam?.vloga} placeholder="npr. vodja nabave" /></label><label>Telefon<input name="telefon" defaultValue={kontaktUrejam?.telefon} /></label><label>E-pošta<input name="email" type="email" defaultValue={kontaktUrejam?.email} /></label><div className={styles.clientEditorActions}><button type="button" onClick={() => { setKontaktOdprt(false); setKontaktUrejam(null); }}>Prekliči</button><button>Shrani osebo</button></div></form></div>}{kontakti.length ? <div className={styles.clientLinkSeznam}>{kontakti.map(k => <div key={k.id} className={styles.clientLinkVrstica} style={{ flexDirection: 'column', alignItems: 'stretch', gap: '.35rem' }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '.5rem' }}><span style={{ display: 'flex', flexDirection: 'column', lineHeight: 1.3 }}><strong>{k.ime}</strong>{k.vloga && <small style={{ fontWeight: 500, color: 'var(--ink)' }}>{k.vloga}</small>}</span>{!samoOgled && <span style={{ display: 'flex', gap: '.25rem', flexShrink: 0 }}><button type="button" className={styles.clientLinkGumb} onClick={() => { setKontaktUrejam(k); setKontaktOdprt(true); }}>Uredi</button><button type="button" className={styles.clientLinkBrisi} onClick={() => izbrisiKontakt(k.id)} aria-label={`Izbriši kontakt ${k.ime}`}>×</button></span>}</div>{(k.telefon || k.email) && <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.4rem' }}>{k.telefon && <a href={`tel:${k.telefon.replace(/\s+/g, '')}`} className={styles.clientDnevnikProj} onClick={() => beleziStik(k, 'klic')}><Phone size={13} weight="bold" style={IKONA_SLOG} aria-hidden="true" /> {k.telefon}</a>}{k.email && <a href={`mailto:${k.email}`} className={styles.clientDnevnikProj} onClick={() => beleziStik(k, 'epošta')}><EnvelopeSimple size={13} weight="bold" style={IKONA_SLOG} aria-hidden="true" /> {k.email}</a>}</div>}</div>)}</div> : <p className={styles.clientLinkPrazno}>Še ni dodanih oseb.</p>}{samoOgled && kontaktOdprt && <p className={styles.clientLinkNamig}>Dodajanje kontaktov ni na voljo v predogledu (demo). Prijavi se v svoj račun.</p>}</div><div className={styles.clientProjects}><h3>Projekti stranke</h3>{selectedProjects.length ? <div className={styles.clientProjectList}>{selectedProjects.map(offer => { const info = projectStatusInfo(offer.status); return <Link key={offer.id} href={`${base}/kalkulator/projekti?projekt=${offer.id}`} className={styles.clientProjectRow}><span className={styles.clientProjectVrh}><strong>{offer.title}</strong><strong className={styles.clientProjectZnesek}>{offer.agreedAmount ? money(offer.agreedAmount) : '—'}</strong></span><span className={styles.clientProjectDno}><small>{datStr(offer.date)}</small><i className={styles.clientProjectPika} data-tone={info.tone}>{info.label}</i></span></Link>; })}</div> : <p className={styles.clientProjectPrazno}>Ta stranka še nima projektov.</p>}</div><div className={styles.clientDnevnik}><div className={styles.clientDnevnikGlava}><div><h3>Dnevnik stranke</h3><small className={styles.clientLinkPodnaslov}>Klici, sestanki, dogovori — kronologija odnosa. Tisto, kar Gmail search ne najde.</small></div>{!samoOgled && <button type="button" className={styles.clientLinkGumb} onClick={() => setDnOdprt(p => !p)}>{dnOdprt ? 'Prekliči' : '+ Dodaj zapis'}</button>}</div>{!samoOgled && dnOdprt && <div className={styles.clientDnevnikObrazec}><div className={styles.clientDnevnikTipi} role="group" aria-label="Vrsta zapisa">{DNEVNIK_TIPI.map(t => <button key={t.tip} type="button" className={dnTip === t.tip ? styles.clientDnevnikTipOn : ''} onClick={() => setDnTip(t.tip)}>{t.label}</button>)}</div><div className={styles.clientDnevnikVrsta}><input type="date" value={dnDatum} onChange={e => setDnDatum(e.target.value)} aria-label="Datum" />{selectedProjects.length > 0 && <select value={dnProjekt} onChange={e => setDnProjekt(e.target.value)} aria-label="Projekt (neobvezno)"><option value="">Brez projekta</option>{selectedProjects.map(o => <option key={o.id} value={o.id}>{o.title}</option>)}</select>}</div><textarea value={dnBesedilo} onChange={e => setDnBesedilo(e.target.value)} placeholder="Kaj je bilo dogovorjeno?" rows={3} aria-label="Zapis" /><button type="button" className={styles.clientLinkDodaj} onClick={addDnevnik} disabled={!dnBesedilo.trim()}>Shrani zapis</button></div>}{samoOgled && dnOdprt && <p className={styles.clientLinkNamig}>Dodajanje zapisov ni na voljo v predogledu (demo). Prijavi se v svoj račun.</p>}{dnevnik.length ? <ol className={styles.clientDnevnikSeznam}>{dnevnik.map(v => { const proj = v.projectId ? selectedProjects.find(o => o.id === v.projectId) : null; return <li key={v.id} className={styles.clientDnevnikVnos} data-tip={v.tip}><span className={styles.clientDnevnikPika} aria-hidden /><div className={styles.clientDnevnikVsebina}><div className={styles.clientDnevnikMeta}><strong>{dnevnikTipLabel(v.tip)}</strong><small>{datStr(v.datum)}</small>{proj && <i className={styles.clientDnevnikProj}>{proj.title}</i>}</div><p>{v.besedilo}</p></div>{!samoOgled && <button type="button" className={styles.clientLinkBrisi} onClick={() => removeDnevnik(v.id)} aria-label="Izbriši zapis">×</button>}</li>; })}</ol> : <p className={styles.clientLinkPrazno}>Še ni zapisov. Prvi klic ali sestanek zabeleži tukaj.</p>}</div><div className={styles.clientLinki}><div className={styles.clientLinkGlava}><div><h3>Skupne povezave stranke</h3><small className={styles.clientLinkPodnaslov}>Viri, ki veljajo čez vse projekte (npr. blagovna knjiga, živa spletna stran). Projektni dokumenti so na projektu.</small></div><button type="button" className={styles.clientLinkGumb} onClick={() => setDodajOdprt(prej => !prej)}>{dodajOdprt ? 'Prekliči' : '+ Dodaj povezavo'}</button></div>{links.length ? <div className={styles.clientLinkSeznam}>{links.map((link, index) => <div key={`${link.url}-${index}`} className={styles.clientLinkVrstica}><a href={link.url} target="_blank" rel="noopener noreferrer">{link.oznaka}</a>{!samoOgled && <button type="button" className={styles.clientLinkBrisi} onClick={() => removeClientLink(index)} aria-label={`Izbriši povezavo ${link.oznaka}`}>×</button>}</div>)}</div> : <p className={styles.clientLinkPrazno}>Še ni dodanih povezav.</p>}{!samoOgled && dodajOdprt && <div className={styles.clientLinkObrazec}><input type="text" value={linkOznaka} onChange={event => setLinkOznaka(event.target.value)} placeholder="npr. Figma" aria-label="Oznaka povezave" /><input type="url" value={linkUrl} onChange={event => setLinkUrl(event.target.value)} placeholder="https://…" aria-label="Naslov povezave (Figma, Drive, spletna stran …)" /><button type="button" className={styles.clientLinkDodaj} onClick={addClientLink} disabled={!linkOznaka.trim() || !linkUrl.trim()}>Shrani povezavo</button></div>}{samoOgled && dodajOdprt && <p className={styles.clientLinkNamig}>Dodajanje povezav ni na voljo v predogledu (demo). Prijavi se v svoj račun.</p>}</div><button className={styles.deleteClient} onClick={() => remove(selected)}>Izbriši profil stranke</button></> : <div className={styles.clientProfileEmpty}><span>↗</span><strong>Izberi stranko.</strong><p>Na enem mestu boš videla vse njene dogovore in rezultate.</p></div>}</section></div>
  </div>;
}
