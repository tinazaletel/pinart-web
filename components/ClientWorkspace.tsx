...
export default function ClientWorkspace() {
  ...
  /* Mobilno (en stolpec): ko izbereš stranko, profil pade pod dolg seznam —
     pripelji ga v vidno polje, da klik ni videti kot da se »nič ne zgodi«. */
  useEffect(() => {
    if (selected && typeof window !== 'undefined' && window.matchMedia('(max-width: 980px)').matches) {
      profilRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  }, [selected]);
