  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (kos) {
        ...
        const dropEnd = maxScroll - 80;          // dokonca se 80px pred dnom
        const dropStart = dropEnd - 550;         // padec traja ~550px drsanja
        const d = Math.min(1, Math.max(0, (y - dropStart) / (dropEnd - dropStart)));
        if (d > 0) {
          x += (mouthX - x) * d;
          ty += (mouthY - ty) * d;
