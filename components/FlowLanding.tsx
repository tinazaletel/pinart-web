...
export default function FlowLanding({ locale = 'sl' }: { locale?: string }) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        try { v.currentTime = 0; v.defaultPlaybackRate = 0.7; v.playbackRate = 0.7; } catch {}
        v.play().catch(() => {});
        setPupaHodi(true);
      } else {
        v.pause();
        ...
        <section className="fl-zgodba" id="onas">
          ...
          <div className="fl-pupa-pas" aria-hidden ref={pasRef}>
            ...
            <video ref={pupaRef} className={`fl-pupa${pupaHodi ? ' hodi' : ''}`} muted loop playsInline preload="auto"
              onLoadedMetadata={e => { const v = e.currentTarget as HTMLVideoElement; v.defaultPlaybackRate = 0.7; v.playbackRate = 0.7; }}>
