...
type FormState = { fullName: string; experience: string; country: string; areas: string[]; company: string; tax: string; email: string; phone: string; address: string; bankAccount: string; mailSignature: string };
const empty: FormState = { fullName: '', experience: 'samostojen', country: '', areas: [], company: '', tax: '', email: '', phone: '', address: '', bankAccount: '', mailSignature: '' };
...
export default function ProfileWorkspace({ base }: { base: string }) {
  ...
  useEffect(() => {
    ...
    setForm({ fullName: local.imeUporabnika || '', experience: local.izkusnje || 'samostojen', country: local.custDrzavaMoj || '', areas: Array.isArray(local.obIzbor) ? local.obIzbor : [], company: provider.ime || '', tax: provider.davcna || '', email: provider.email || '', phone: provider.telefon || '', address: provider.naslov || '', bankAccount: provider.trr || '', mailSignature: local.podpisMaila || '' });
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    localStorage.setItem('pinart-kalkulator-v2', JSON.stringify({ ...current, podpisMaila: form.mailSignature, imeUporabnika: form.fullName, izkusnje: form.experience, custDrzavaMoj: form.country, obIzbor: form.areas, ponudnik: { ...(current.ponudnik || {}), ime: form.company, davcna: form.tax, email: form.email, telefon: form.phone, naslov: form.address, trr: form.bankAccount } }));
  ...
  return <div className={styles.page}>{notice && <div className={styles.notice} role="status">{notice}</div>}<form onSubmit={save}>
    ...
    <section className={styles.card}><header><p>03 · PODPIS POŠTE</p><h2>Podpis v e-pošti.</h2><span>Samodejno se doda na dno vsakega novega sporočila iz projekta. E-naslov lahko vključiš — vstavi se kot <b>navadno besedilo (ni klikabilen)</b>, da stranke raje kliknejo »Odgovori« kot pišejo na napačen naslov.</span></header><div className={styles.grid}><label style={{ gridColumn: '1 / -1' }}>Podpis<textarea value={form.mailSignature} onChange={e => field('mailSignature', e.target.value)} rows={5} placeholder={'Tina Zaletel\nPinart · oblikovanje\n+386 40 123 456\ntina@pinart.si\npinart.si'} style={{ width: '100%', padding: '.6rem .7rem', fontFamily: 'inherit', fontSize: '.9rem', lineHeight: 1.5, resize: 'vertical', borderRadius: '.5rem', border: '1px solid rgba(17,17,17,.15)' }} /></label></div></section>
    ...
    <section className={styles.card}>
      <header><p>04 · VIDEZ DOKUMENTOV</p><h2>Barva in pisava dokumentov.</h2><span>Skupni videz vseh dokumentov — ponudb, pogodb in računov. Enako lahko urediš tudi ob generiranem dokumentu.</span></header>
