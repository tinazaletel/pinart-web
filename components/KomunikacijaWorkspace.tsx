...
export default function KomunikacijaWorkspace({ jeEn = false, projektId, projektNaziv, vgrajeno = false, initialId }: { jeEn?: boolean; projektId?: string; projektNaziv?: string; vgrajeno?: boolean; initialId?: string }) {
    ...
    (async () => {
      try {
        ...
        const cloudVnosi: PostaVnos[] = cloud.map(m => ({
          ...
          popravek: m.popravek,
          popravekResenAt: m.popravekResenAt,
        }));
  ...
  const nastaviPopravek = (m: PostaVnos, popravek: boolean, resenAt?: string) => {
    if (!demo) {
      nastaviPopravekPoste(m.id, popravek, resenAt);
      void updateProjectMailRevision(m.id, popravek, resenAt).catch(() => undefined);
    }
    posodobiMail(m.id, { popravek, popravekResenAt: popravek ? resenAt : undefined });
  };
  ...
  return (
      ...
      {zavihek === 'posta' ? (
        <div className={'km-posta-ovoj' + (beriMail ? ' km-bere' : '')}>
          <div className="km-posta-vrh">
            ...
            <button type="button" className="km-mape-trig" onClick={() => setMapeOdprt(true)} aria-label={L('Mape', 'Folders')} aria-expanded={mapeOdprt}><List size={16} weight="bold" /> <span>{mapa === 'pogovori' ? L('Niti', 'Threads') : mapa === 'prejeto' ? L('Prejeto', 'Inbox') : mapa === 'poslano' ? L('Poslano', 'Sent') : mapa === 'osnutki' ? L('Osnutki', 'Drafts') : L('Koš', 'Trash')}</span></button>
          ...
          <div className="km-posta-body">
            ...
            <aside className={'km-mape' + (mapeOdprt ? ' odprt' : '')} aria-label={L('Mape', 'Folders')}>
              {([{ id: 'pogovori', ime: L('Niti', 'Threads'), I: ChatsCircle }, { id: 'popravki', ime: L('Popravki', 'Revisions'), I: CheckSquare }, { id: 'prejeto', ime: L('Prejeto', 'Inbox'), I: Tray }, { id: 'poslano', ime: L('Poslano', 'Sent'), I: PaperPlaneRight }, { id: 'osnutki', ime: L('Osnutki', 'Drafts'), I: NotePencil }, { id: 'kos', ime: L('Koš', 'Trash'), I: Trash }] as const).map(({ id, ime, I }) => {
                const st = id === 'pogovori' ? postaNiti.length : id === 'popravki' ? vidnaPosta.filter(v => v.popravek && !v.popravekResenAt && !v.izbrisano).length : vidnaPosta.filter(v => (v.izbrisano ? 'kos' : v.osnutek ? 'osnutki' : v.smer === 'poslano' ? 'poslano' : 'prejeto') === id).length;
                  ...
                  <span className="km-ik-w">
                    ...
                    {oznakaOdprt && (
                      <div className="km-meni" role="menu">
                        ...
                        <button type="button" className="km-popravek-preklop" onClick={() => nastaviPopravek(beriMail, !beriMail.popravek)}><CheckSquare size={15} weight={beriMail.popravek ? 'fill' : 'regular'} /> {beriMail.popravek ? L('Odstrani oznako Popravek', 'Remove Revision label') : L('Označi kot Popravek', 'Mark as Revision')}</button>
   ...
   "Oznaci vse" poravna s kljukicami v njem. */
        ...
        .km-posta .km-mail-vrsta{display:flex;align-items:center;gap:.75rem;width:100%;text-align:left;background:rgba(255,255,255,.5);backdrop-filter:blur(14px) saturate(1.3);-webkit-backdrop-filter:blur(14px) saturate(1.3);border:1px solid color-mix(in oklch,var(--ink) 9%,transparent);border-radius:.85rem;padding:.7rem .9rem;cursor:pointer;transition:background .16s ease,box-shadow .16s ease,transform .16s ease}
   ...
   nad njim, torej do desnega roba kosa. */
        ...
        .km-popravek-stanje,.km-popravek-oznaka{display:inline-flex;align-items:center;gap:.35rem;width:max-content;border-radius:999px;background:color-mix(in oklch,var(--k-purple) 11%,#fff);color:#5b408e;font:700 .72rem var(--font-sans),sans-serif}
        .km-popravek-stanje{margin-top:.45rem;padding:.35rem .65rem;cursor:pointer}
        .km-popravek-stanje input{accent-color:#6E4FA6}
        .km-popravek-oznaka{margin-top:.28rem;padding:.18rem .48rem}
