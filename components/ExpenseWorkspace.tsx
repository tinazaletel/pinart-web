...
export default function ExpenseWorkspace() {
  ...
  return <div className={styles.expensePage}>
    ...
    <section className={styles.expenseArchive}><header><div><p className={styles.eyebrow}>EVIDENCA</p><h2>Stroški po namenu.</h2></div><div className={styles.invoiceFilters}>{(['all', 'project', 'company'] as const).map(value => <button key={value} className={filter === value ? styles.invoiceFilterActive : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'Vsi' : value === 'project' ? 'Projektni' : 'Podjetje'}</button>)}</div></header>{filterTags.length > 0 && <div className={`${styles.invoiceFilters} ${styles.tagFilterRow}`}><button className={tagFilter === null ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(null)}>Vse značke</button>{filterTags.map(tag => <button key={tag} className={tagFilter && tagFilter.toLowerCase() === tag.toLowerCase() ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(current => current && current.toLowerCase() === tag.toLowerCase() ? null : tag)}>{tag}</button>)}</div>}{shown.length > 0 && <label className={styles.expenseSelAll}><input type="checkbox" className={styles.expenseChk} checked={vsiIzbrani} onChange={preklopiVse} aria-label="Označi vse" /><span>{vsiIzbrani ? 'Odznači vse' : `Označi vse (${shown.length})`}</span></label>}{shown.length ? <div className={styles.expenseList}>{shown.map(item => { const offer = offers.find(offerItem => offerItem.id === item.sourceOfferId); return <article key={item.id} className={izbrani.has(item.id) ? styles.expenseRowSel : undefined}><input type="checkbox" className={styles.expenseChk} checked={izbrani.has(item.id)} onChange={() => preklopiIzbor(item.id)} aria-label={`Izberi ${item.title}`} /><span className={styles.expenseCoin}>€</span><div><strong>{item.title}</strong><small>{item.category || 'Strošek'}{jeReden(item) ? ` · ${item.obdobje === 'letni' ? 'letno' : 'mesečno'}` : ''} · {new Date(item.date).toLocaleDateString('sl-SI')}</small>{item.tags && item.tags.length > 0 && <div className={styles.expenseItemTags}>{item.tags.map(tag => <span key={tag} className={styles.expenseItemTag}>{tag}</span>)}</div>}</div><div><strong>{money(item.amount)}</strong><small>{offer ? `Projekt: ${offer.title}` : item.company ? `Podjetje: ${item.company}` : 'Splošni strošek'}</small></div><div className={styles.expenseActions}><button className={styles.expenseEdit} onClick={() => { setEditing(item); setOpen(true); }} aria-label={`Uredi ${item.title}`}><PencilSimple size={15} weight="bold" /></button><button className={styles.expenseDelete} onClick={() => remove(item.id)} aria-label={`Izbriši ${item.title}`}><X size={14} weight="bold" /></button></div></article>; })}</div> : <p className={styles.invoiceEmpty}>V tem pogledu še ni stroškov.</p>}</section>
