...
export default function ExpenseWorkspace() {
  ...
  const izvoziCsv = () => {
    ...
    const glava = ['Naziv', 'Kategorija', 'Obdobje', 'Datum', 'Značke', 'Znesek', 'Valuta', 'Namen'];
  ...
  return <div className={styles.expensePage + (open ? ' expense-editing' : '')}>
    ...
    <section className={styles.expenseSummary}><article><small>Ta mesec</small><strong>{money(monthTotal)}</strong>{monthTuje ? <span>+ {monthTuje} · ni preračunano</span> : null}<span>{expenses.length} evidentiranih stroškov</span><b className={styles.subpageMetricIcon}><MetricIcon type="cost" /></b></article><article><small>Redni mesečni</small><strong>{money(recurringTotal)}</strong>{recurringTuje ? <span>+ {recurringTuje} · ni preračunano</span> : null}<span>redni stroški (mesečni + letni/12)</span><b className={styles.subpageMetricIcon}><MetricIcon type="recurring" /></b></article><article><small>Skupaj osnova</small><strong>{money(monthTotal + recurringTotal)}</strong>{skupajTuje ? <span>+ {skupajTuje} · ni preračunano</span> : null}<span>poslovni in projektni stroški</span><b className={styles.subpageMetricIcon}><MetricIcon type="profit" /></b></article><button onClick={() => { setEditing(null); setOpen(true); }}><span>+</span><strong>Dodaj strošek</strong><small>Projekt, podjetje ali splošno</small></button></section>
    {open && <section ref={editorRef} className={styles.expenseEditor}><button type="button" className={styles.expenseBack} onClick={() => { setOpen(false); setEditing(null); }} aria-label="Nazaj na stroške"><CaretLeft size={16} weight="bold" /> Stroški</button><div><p className={styles.eyebrow}>{editing ? 'UREDI STROŠEK' : 'NOV STROŠEK'}</p><h2>Kam ga pripišemo?</h2><p>Projektni stroški vplivajo na donosnost stranke. Splošni ostanejo strošek podjetja.</p></div><form onSubmit={save}><label>Opis<input required name="title" defaultValue={editing?.title} /></label><label>Kategorija<select name="category" defaultValue={editing?.category || 'Drugo'}><option>Programska oprema</option><option>Sodelavec / partner</option><option>Produkcija</option><option>Oprema</option><option>Potni stroški</option><option>Drugo</option></select></label><label>Obdobje<select name="obdobje" defaultValue={editing?.obdobje || 'enkratni'}><option value="enkratni">Enkratni</option><option value="mesecni">Mesečni (redni)</option><option value="letni">Letni (redni)</option></select></label><label className={styles.expenseTagsLabel}>Značke<div className={styles.expenseTagsBox}>{tags.map(tag => <span key={tag} className={styles.expenseTagChip}>{tag}<button type="button" onClick={() => odstraniZnacko(tag)} aria-label={`Odstrani značko ${tag}`}>×</button></span>)}<input value={tagInput} onChange={event => setTagInput(event.target.value)} onKeyDown={onTagKeyDown} placeholder={tags.length ? '' : 'npr. mesečni, najem…'} /></div></label>{vidniPredlogi.length > 0 && <div className={styles.expenseTagSuggestions}>{vidniPredlogi.map(tag => <button key={tag} type="button" onClick={() => dodajZnacko(tag)}>{tag}</button>)}</div>}<label>Znesek<span className={styles.expenseZnesek} title="Znesek vpišeš neposredno v izbrani valuti — brez preračuna tečaja"><input required name="amount" type="number" min="0" step="0.01" defaultValue={editing?.amount} /><select name="valuta" defaultValue={editing?.valuta || 'eur'} aria-label="Valuta stroška">{VALUTE_RACUN.map(v => <option key={v.id} value={v.id}>{v.id.toUpperCase()} {v.znak}</option>)}</select></span></label><label>Datum<input required name="date" type="date" defaultValue={editing?.date || new Date().toISOString().slice(0, 10)} /></label><label>Račun ali potrdilo<span className={styles.expenseDatoteka}><input ref={datotekaRef} name="document" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" onChange={e => setImeDatoteke(e.target.files?.[0]?.name || '')} />{imeDatoteke && <button type="button" className={styles.expenseDatotekaX} onClick={e => { e.preventDefault(); e.stopPropagation(); setImeDatoteke(''); if (datotekaRef.current) datotekaRef.current.value = ''; }} aria-label={`Odstrani datoteko ${imeDatoteke}`} title="Odstrani datoteko">×</button>}</span>{editing?.fileName && <small>Trenutno: {editing.fileName}</small>}</label><label>Projekt / ponudba<select name="sourceOfferId" defaultValue={editing?.sourceOfferId || ''}><option value="">Ni vezan na projekt</option>{offers.map(offer => <option key={offer.id} value={offer.id}>{offer.title} · {offer.client}</option>)}</select></label><label>Podjetje<select name="company" defaultValue={editing?.company || ''}><option value="">Moje glavno podjetje</option>{companies.map(company => <option key={company}>{company}</option>)}</select></label><label>Stranka brez ponudbe<input name="client" defaultValue={editing?.client} placeholder="neobvezno" /></label><div className={styles.expenseEditorActions}>{editing && <button type="button" className={styles.expenseEditorDelete} onClick={() => { if (editing && remove(editing.id)) { setOpen(false); setEditing(null); } }} aria-label="Izbriši strošek" title="Izbriši strošek"><Trash size={16} weight="bold" /></button>}<button type="button" onClick={() => { setOpen(false); setEditing(null); }}>Prekliči</button><button>{editing ? 'Shrani spremembe' : 'Shrani strošek'}</button></div></form></section>}
    <section className={styles.expenseArchive}><header><div><p className={styles.eyebrow}>EVIDENCA</p><h2>Stroški po namenu.</h2></div><div className={styles.expenseHeadRow}><MobTabs label="Prikaz" vrednost={filter} naVrednost={id => setFilter((id || 'all') as 'all' | 'project' | 'company')} opcije={[{ id: 'all', label: 'Vsi' }, { id: 'project', label: 'Projektni' }, { id: 'company', label: 'Podjetje' }]} /><div className={`${styles.invoiceFilters} mobtabs-hide`}>{(['all', 'project', 'company'] as const).map(value => <button key={value} className={filter === value ? styles.invoiceFilterActive : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'Vsi' : value === 'project' ? 'Projektni' : 'Podjetje'}</button>)}</div>{filterTags.length > 0 && <MobTabs label="Značke" vrednost={tagFilter ?? ''} naVrednost={id => setTagFilter(id || null)} opcije={[{ id: '', label: 'Vse značke' }, ...filterTags.map(tag => ({ id: tag, label: tag }))]} />}{filterTags.length > 0 && <div className={`${styles.invoiceFilters} ${styles.tagFilterRow} mobtabs-hide`}><button className={tagFilter === null ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(null)}>Vse značke</button>{filterTags.map(tag => <button key={tag} className={tagFilter && tagFilter.toLowerCase() === tag.toLowerCase() ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(current => current && current.toLowerCase() === tag.toLowerCase() ? null : tag)}>{tag}</button>)}</div>}<div style={{ display: 'flex' }}><IskalnikMob vrednost={search} naVrednost={setSearch} placeholder="Poišči strošek …" label="Poišči strošek" /></div></div></header>{shown.length > 0 && <div className={styles.expenseSelAllRow}><label className={styles.expenseSelAll}><input type="checkbox" className={styles.expenseChk} checked={vsiIzbrani} onChange={preklopiVse} aria-label="Označi vse" /><span>{vsiIzbrani ? 'Odznači vse' : `Označi vse (${shown.length})`}</span></label>{izbrani.size > 0 && <button type="button" className={styles.expenseBulkDelete} onClick={removeIzbrane} aria-label={`Izbriši izbrane (${izbrani.size})`} title="Izbriši izbrane"><Trash size={15} weight="bold" /></button>}</div>}{shown.length ? <><div className={styles.expenseList}>{shownStran.map(item => { const offer = offers.find(offerItem => offerItem.id === item.sourceOfferId); return <div key={item.id} className={styles.expenseSwipe} ref={el => { if (el) swipeEls.current.set(item.id, el); else swipeEls.current.delete(item.id); }} onScroll={onSwipeScroll(item.id)} onClick={onRowClick(item)}><article className={izbrani.has(item.id) ? styles.expenseRowSel : undefined}><input type="checkbox" className={styles.expenseChk} data-row-select checked={izbrani.has(item.id)} onChange={() => preklopiIzbor(item.id)} onClick={event => event.stopPropagation()} aria-label={`Izberi ${item.title}`} /><span className={styles.expenseCoin}>{valutaZnak(item.valuta || 'eur')}</span><div><strong>{item.title}</strong><small>{item.category || 'Strošek'}{jeReden(item) ? ` · ${item.obdobje === 'letni' ? 'letno' : 'mesečno'}` : ''} · {new Date(item.date).toLocaleDateString('sl-SI')}</small></div><div><strong>{money(item.amount, item.valuta)}</strong><small>{offer ? `Projekt: ${offer.title}` : item.company ? `Podjetje: ${item.company}` : 'Splošni strošek'}</small></div><div className={styles.expenseActions}><button className={styles.expenseKebab} onClick={onDotsClick(item.id)} aria-label={`Dejanja: ${item.title}`} aria-expanded={kebabId === item.id}><DotsThreeVertical size={18} weight="bold" /></button>{kebabId === item.id && <><div className={styles.expenseKebabBack} onClick={() => setKebabId(null)} aria-hidden /><div className={styles.expenseKebabMenu} role="menu"><button type="button" onClick={() => { setEditing(item); setOpen(true); setKebabId(null); }}><PencilSimple size={14} weight="bold" /> Uredi</button><button type="button" className={styles.expenseKebabBrisi} onClick={() => { remove(item.id); setKebabId(null); }}><X size={13} weight="bold" /> Izbriši</button></div></>}</div>{item.tags && item.tags.length > 0 && <div className={styles.expenseItemTags}>{item.tags.map(tag => <span key={tag} className={styles.expenseItemTag} style={{ '--tag-h': [...tag].reduce((a, c) => a + c.charCodeAt(0), 0) % 360 } as CSSProperties}>{tag}</span>)}</div>}</article><div className={styles.expenseSwipeActions} data-swipe-actions><button type="button" className={styles.expenseSwipeBrisi} onClick={() => remove(item.id)} aria-label={`Izbriši ${item.title}`}><Trash size={17} weight="bold" /><span>Izbriši</span></button></div></div>; })}</div><Paginacija stran={stranStrVarno} strani={straniStr} naStran={setStranStr} /></> : (expenses.length === 0
      ? <div className={styles.expenseEmpty}><span className={styles.expenseEmptyIkona}><Receipt size={24} weight="light" /></span><strong>Še ni stroškov.</strong><p>Dodaj prvega z enotnim gumbom »Dodaj strošek« zgoraj.</p></div>
