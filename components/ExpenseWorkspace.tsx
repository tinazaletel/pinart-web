...
export default function ExpenseWorkspace() {
  ...
  return <div className={styles.expensePage}>
    ...
    <section className={styles.expenseArchive}><header><div><p className={styles.eyebrow}>EVIDENCA</p><h2>Stroški po namenu.</h2></div><div className={styles.invoiceFilters}>{(['all', 'project', 'company'] as const).map(value => <button key={value} className={filter === value ? styles.invoiceFilterActive : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'Vsi' : value === 'project' ? 'Projektni' : 'Podjetje'}</button>)}</div></header>{shown.length ? <div className={styles.expenseList}>{shown.map(item => { const offer = offers.find(offerItem => offerItem.id === item.sourceOfferId); return <article key={item.id}><span className={styles.expenseCoin}>€</span><div><strong>{item.title}</strong><small>{item.category || 'Strošek'} · {new Date(item.date).toLocaleDateString('sl-SI')}</small></div><div><strong>{money(item.amount)}</strong><small>{offer ? `Projekt: ${offer.title}` : item.company ? `Podjetje: ${item.company}` : 'Splošni strošek'}</small></div><div className={styles.expenseActions}><button className={styles.expenseEdit} onClick={() => { setEditing(item); setOpen(true); }} aria-label={`Uredi ${item.title}`}><PencilSimple size={15} weight="bold" /></button><button className={styles.expenseDelete} onClick={() => remove(item.id)} aria-label={`Izbriši ${item.title}`}><X size={14} weight="bold" /></button></div></article>; })}</div> : <p className={styles.invoiceEmpty}>V tem pogledu še ni stroškov.</p>}</section>
