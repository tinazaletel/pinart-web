...
export default function ExpenseWorkspace() {
  ...
  return <div className={styles.expensePage}>
    ...
    <section className={styles.expenseArchive}><header><div><p className={styles.eyebrow}>EVIDENCA</p><h2>Stroški po namenu.</h2></div><div style={{ position: 'relative', display: 'flex', flexWrap: 'wrap', gap: '.5rem', alignItems: 'center', width: '100%' }}><IskalnikMob vrednost={search} naVrednost={setSearch} placeholder="Poišči strošek …" label="Poišči strošek" /><MobTabs label="Prikaz" vrednost={filter} naVrednost={id => setFilter((id || 'all') as 'all' | 'project' | 'company')} opcije={[{ id: 'all', label: 'Vsi' }, { id: 'project', label: 'Projektni' }, { id: 'company', label: 'Podjetje' }]} /><div className={`${styles.invoiceFilters} mobtabs-hide`}>{(['all', 'project', 'company'] as const).map(value => <button key={value} className={filter === value ? styles.invoiceFilterActive : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'Vsi' : value === 'project' ? 'Projektni' : 'Podjetje'}</button>)}</div>{filterTags.length > 0 && <MobTabs label="Značke" vrednost={tagFilter ?? ''} naVrednost={id => setTagFilter(id || null)} opcije={[{ id: '', label: 'Vse značke' }, ...filterTags.map(tag => ({ id: tag, label: tag }))]} />}</div></header>
{filterTags.length > 0 && <div className={`${styles.invoiceFilters} ${styles.tagFilterRow} mobtabs-hide`}><button className={tagFilter === null ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(null)}>Vse značke</button>{filterTags.map(tag => <button key={tag} className={tagFilter && tagFilter.toLowerCase() === tag.toLowerCase() ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(current => current && current.toLowerCase() === tag.toLowerCase() ? null : tag)}>{tag}</button>)}</div>}{shown.length > 0 && <label className={styles.expenseSelAll}><input type="checkbox" className={styles.expenseChk} checked={vsiIzbrani} onChange={preklopiVse} aria-label="Označi vse" /><span>{vsiIzbrani ? 'Odznači vse' : `Označi vse (${shown.length})`}</span></label>}{shown.length ? <><div className={styles.expenseList}>{shownStran.map(item => { const offer = offers.find(offerItem => offerItem.id === item.sourceOfferId); return <article key={item.id} className={izbrani.has(item.id) ? styles.expenseRowSel : undefined}><input type="checkbox" className={styles.expenseChk} checked={izbrani.has(item.id)} onChange={() => preklopiIzbor(item.id)} aria-label={`Izberi ${item.title}`} /><span className={styles.expenseCoin}>€</span><div><strong>{item.title}</strong><small>{item.category || 'Strošek'}{jeReden(item) ? ` · ${item.obdobje === 'letni' ? 'letno' : 'mesečno'}` : ''} · {new Date(item.date).toLocaleDateString('sl-SI')}</small>{item.tags && item.tags.length > 0 && <div className={styles.expenseItemTags}>{item.tags.map(tag => <span key={tag} className={styles.expenseItemTag}>{tag}</span>)}</div>}</div><div><strong>{money(item.amount)}</strong><small>{offer ? `Projekt: ${offer.title}` : item.company ? `Podjetje: ${item.company}` : 'Splošni strošek'}</small></div><div className={styles.expenseActions}><button className={styles.expenseKebab} onClick={() => setKebabId(prev => prev === item.id ? null : item.id)} aria-label={`Dejanja: ${item.title}`} aria-expanded={kebabId === item.id}><DotsThree size={18} weight="bold" /></button>{kebabId === item.id && <><div className={styles.expenseKebabBack} onClick={() => setKebabId(null)} aria-hidden /><div className={styles.expenseKebabMenu} role="menu"><button type="button" onClick={() => { setEditing(item); setOpen(true); setKebabId(null); }}><PencilSimple size={14} weight="bold" /> Uredi</button><button type="button" className={styles.expenseKebabBrisi} onClick={() => { remove(item.id); setKebabId(null); }}><X size={13} weight="bold" /> Izbriši</button></div></>}</div></article>; })}</div><Paginacija stran={stranStrVarno} strani={straniStr} naStran={setStranStr} /></> : <p className={styles.invoiceEmpty}>V tem pogledu še ni stroškov.</p>}</section>
