...
export default function ExpenseWorkspace() {
  ...
  const shown = expenses.filter(item => (filter === 'all' || (filter === 'project' ? item.sourceOfferId : item.company)) && (!tagFilter || (item.tags || []).some(t => t.toLowerCase() === tagFilter.toLowerCase())));
  ...
  return <div className={styles.expensePage}>
    ...
    {open && <section ref={editorRef} className={styles.expenseEditor}><div><p className={styles.eyebrow}>{editing ? 'UREDI STROŠEK' : 'NOV STROŠEK'}</p><h2>Kam ga pripišemo?</h2><p>Projektni stroški vplivajo na donosnost stranke. Splošni ostanejo strošek podjetja.</p></div><form onSubmit={save}><label>Opis<input required name="title" defaultValue={editing?.title} /></label><label>Kategorija<select name="category" defaultValue={editing?.category || 'Drugo'}><option>Programska oprema</option><option>Zunanji sodelavec</option><option>Produkcija</option><option>Oprema</option><option>Potni stroški</option><option>Drugo</option></select></label><label className={styles.expenseTagsLabel}>Značke<div className={styles.expenseTagsBox}>{tags.map(tag => <span key={tag} className={styles.expenseTagChip}>{tag}<button type="button" onClick={() => odstraniZnacko(tag)} aria-label={`Odstrani značko ${tag}`}>×</button></span>)}<input value={tagInput} onChange={event => setTagInput(event.target.value)} onKeyDown={onTagKeyDown} placeholder={tags.length ? '' : 'npr. mesečni, najem…'} /></div>{vidniPredlogi.length > 0 && <div className={styles.expenseTagSuggestions}>{vidniPredlogi.map(tag => <button key={tag} type="button" onClick={() => dodajZnacko(tag)}>{tag}</button>)}</div>}</label><label>Znesek<input required name="amount" type="number" min="0" step="0.01" defaultValue={editing?.amount} /></label><label>Datum<input required name="date" type="date" defaultValue={editing?.date || new Date().toISOString().slice(0, 10)} /></label><label>Račun ali potrdilo<input name="document" type="file" accept=".pdf,.jpg,.jpeg,.png,.webp" />{editing?.fileName && <small>Trenutno: {editing.fileName}</small>}</label><label>Projekt / ponudba<select name="sourceOfferId" defaultValue={editing?.sourceOfferId || ''}><option value="">Ni vezan na projekt</option>{offers.map(offer => <option key={offer.id} value={offer.id}>{offer.title} · {offer.client}</option>)}</select></label><label>Podjetje<select name="company" defaultValue={editing?.company || ''}><option value="">Moje glavno podjetje</option>{companies.map(company => <option key={company}>{company}</option>)}</select></label><label>Stranka brez ponudbe<input name="client" defaultValue={editing?.client} placeholder="neobvezno" /></label><div className={styles.expenseEditorActions}><button type="button" onClick={() => { setOpen(false); setEditing(null); }}>Prekliči</button><button>{editing ? 'Shrani spremembe' : 'Shrani strošek'}</button></div></form></section>}
    <section className={styles.expenseArchive}><header><div><p className={styles.eyebrow}>EVIDENCA</p><h2>Stroški po namenu.</h2></div><div className={styles.invoiceFilters}>{(['all', 'project', 'company'] as const).map(value => <button key={value} className={filter === value ? styles.invoiceFilterActive : ''} onClick={() => setFilter(value)}>{value === 'all' ? 'Vsi' : value === 'project' ? 'Projektni' : 'Podjetje'}</button>)}</div></header>{filterTags.length > 0 && <div className={`${styles.invoiceFilters} ${styles.tagFilterRow}`}><button className={tagFilter === null ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(null)}>Vse značke</button>{filterTags.map(tag => <button key={tag} className={tagFilter && tagFilter.toLowerCase() === tag.toLowerCase() ? styles.invoiceFilterActive : ''} onClick={() => setTagFilter(current => current && current.toLowerCase() === tag.toLowerCase() ? null : tag)}>{tag}</button>)}</div>}{shown.length ? <div className={styles.expenseList}>{shown.map(item => { const offer = offers.find(offerItem => offerItem.id === item.sourceOfferId); return <article key={item.id}><span className={styles.expenseCoin}>€</span><div><strong>{item.title}</strong><small>{item.category || 'Strošek'} · {new Date(item.date).toLocaleDateString('sl-SI')}</small>{item.tags && item.tags.length > 0 && <div className={styles.expenseItemTags}>{item.tags.map(tag => <span key={tag} className={styles.expenseItemTag}>{tag}</span>)}</div>}</div><div><strong>{money(item.amount)}</strong><small>{offer ? `Projekt: ${offer.title}` : item.company ? `Podjetje: ${item.company}` : 'Splošni strošek'}</small></div><div className={styles.expenseActions}><button className={styles.expenseEdit} onClick={() => { setEditing(item); setOpen(true); }} aria-label={`Uredi ${item.title}`}><PencilSimple size={15} weight="bold" /></button><button className={styles.expenseDelete} onClick={() => remove(item.id)} aria-label={`Izbriši ${item.title}`}><X size={14} weight="bold" /></button></div></article>; })}</div> : <p className={styles.invoiceEmpty}>V tem pogledu še ni stroškov.</p>}</section>
