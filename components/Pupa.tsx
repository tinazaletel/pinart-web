...
const OBRAZ = (px: number) => <PupaObraz px={px} />;
...
export default function Pupa() {
  ...
  /* V OKVIRJU (iframe pogojev v kartici) Pupinega sprozilca NI: fiksni sparkle
     je plaval cez besedilo pogojev in izgledal kot napaka — ravno na zaslonu,
     ki prodaja zaupanje (Tina, 25. 8.: "ai ikonica je tu po pomoti"). */
  ...
  return createPortal(
    <>
      ...
      <style>{'.pupa-vnos{scrollbar-width:thin;scrollbar-color:rgba(42,32,53,.22) transparent}.pupa-vnos::-webkit-scrollbar{width:6px}.pupa-vnos::-webkit-scrollbar-track{background:transparent}.pupa-vnos::-webkit-scrollbar-thumb{background:rgba(42,32,53,.2);border-radius:99px}'+'body:has(.izbirnik-zastor) .pupa-fab,body:has(.izbirnik-plosca) .pupa-fab,body:has(.soglasje) .pupa-fab,body:has(.dp) .pupa-fab{display:none!important}.pupa-fab-mini{display:none}@media (max-width:760px){.pupa-fab{top:.55rem!important;bottom:auto!important;right:5rem!important;width:2rem!important;height:2rem!important;background:transparent!important;border:0!important;box-shadow:none!important;display:flex!important;align-items:center!important;justify-content:center!important}.pupa-fab .pupa-fab-full{display:none!important}.pupa-fab .pupa-fab-mini{display:block!important}}'}</style>
      {!odprt && (
      ...
      {odprt && (
        <div role="dialog" aria-label="Pupa" style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 'min(400px, 94vw)', zIndex: 95, background: '#fff', borderLeft: '1px solid rgba(42,32,53,.12)', boxShadow: '-16px 0 50px rgba(42,32,53,.18)', display: 'flex', flexDirection: 'column', color: '#2A2035' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', padding: '1rem 1.1rem', borderBottom: '1px solid rgba(42,32,53,.08)' }}>
            ...
            <button type="button" onClick={() => { if (typeof window !== 'undefined') { if (zvok) { window.speechSynthesis?.cancel(); } else { try { window.speechSynthesis?.resume(); const o = new SpeechSynthesisUtterance(' '); o.volume = 0; window.speechSynthesis?.speak(o); } catch { /* ignore */ } } } setZvok(z => !z); }} aria-pressed={zvok} aria-label={zvok ? L('Utišaj Pupo', 'Mute Pupa') : L('Vklopi glas Pupe', 'Unmute Pupa')} title={zvok ? L('Pupa bere odgovore na glas — utišaj tukaj', 'Pupa reads answers aloud — mute here') : L('Vklopi, da Pupa bere odgovore na glas', 'Turn on so Pupa reads answers aloud')} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 4, color: zvok ? '#2A2035' : 'rgba(42,32,53,.4)', display: 'inline-flex' }}>
