...
export default function PitchAgent({ base = '', onSave }: PitchAgentProps) {
  ...
  return <div className="pa">
    ...
    {!projekti.length ? <p className="pa-prazno">{L('Najprej potrebuješ projekt.', 'You need a project first.')} <a href={`${base}/kalkulator/nov-projekt`}>{L('Ustvari projekt', 'Create project')}</a></p> : <>
      <div className="pa-pot" role="group" aria-label={L('Način priprave', 'Preparation method')}><button type="button" className={pot === 'ai' ? 'on' : ''} onClick={() => setPot('ai')}>{L('Naj napiše Pupa', 'Let Pupa write')}</button><button type="button" className={pot === 'rocno' ? 'on' : ''} onClick={() => { setPot('rocno'); setIzid(v => v || { naslov: '', problem: '', resitev: '', zakajMi: '', obseg: '', okvirnaCena: '', naslednjiKorak: '' }); }}>{L('Izpolnim sama', 'I’ll fill it in')}</button></div>
      ...
      {pot === 'ai' && <label><span>{L('Kaj ponujaš in komu?', 'What are you offering, and to whom?')}</span><textarea rows={5} value={opis} onChange={e => setOpis(e.target.value)} placeholder={L('Opiši priložnost, strankin problem, želeni obseg in morebitni cenovni okvir.', 'Describe the opportunity, client problem, scope, and any price range.')} /></label>}
      <div className="pa-akcije">{pot === 'ai' && <button type="button" className="pa-glavni" onClick={napisi} disabled={!opis.trim() || !projekt || dela}>{dela ? L('Pišem …', 'Writing …') : L('Napiši pitch', 'Write pitch')}</button>}{izid && <button type="button" className="pa-drugi" onClick={shrani}>{L('Shrani', 'Save')}</button>}</div>
      ...
      {izid && <section className="pa-izid">{polja.map(([k, oznaka]) => { const pomoc = ({ naslov: L('Npr. Identiteta, ki napolni hotel.', 'E.g. An identity that fills the hotel.'), problem: L('Kaj stranko ovira?', 'What blocks the client?'), resitev: L('Kako boš to rešila?', 'How will you solve it?'), zakajMi: L('Zakaj si prava izbira?', 'Why are you the right choice?'), obseg: L('Kaj je vključeno?', 'What is included?'), okvirnaCena: L('Cena ali način določitve.', 'Price or how it is set.'), naslednjiKorak: L('Kaj naj naredi zdaj?', 'What should they do next?') } as Record<keyof PitchPolja, string>)[k]; return <label key={k}><span>{oznaka}<small>{pomoc}</small></span>{k === 'naslov' ? <input placeholder={pomoc} value={izid[k]} onChange={e => setIzid({ ...izid, [k]: e.target.value })} /> : <textarea placeholder={pomoc} rows={3} value={izid[k]} onChange={e => setIzid({ ...izid, [k]: e.target.value })} />}</label>; })}</section>}
    </>}
