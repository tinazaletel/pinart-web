...
export default function DashboardHeaderTools() {
  ...
  return <div className={styles.headerTools} aria-label="Uporabniške nastavitve">
    ...
    {aiOpen && <div className={`${styles.headerPopover} ${styles.aiPopover}`}><strong>Pupa · AI asistentka</strong>{hasAi ? <><p>Pomaga ti razumeti rezultate, pripraviti osnutke in najti naslednji korak — lahko tudi z glasom.</p><button type="button" className={styles.aiCta} onClick={() => { setAiOpen(false); if (typeof window !== 'undefined') window.dispatchEvent(new CustomEvent('pupa:odpri', { detail: { nacin: 'glas' } })); }}>Začni pogovor</button></> : <><p>AI asistentka Pupa je del plačljivega paketa. Brezplačni kalkulator ostaja brezplačen.</p><Link className={styles.aiCta} href={`${base}/kalkulator/paket`}>Nadgradi paket</Link></>}</div>}
