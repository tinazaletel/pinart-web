...
export default function DashboardHeaderTools() {
  ...
  return <div className={styles.headerTools} aria-label="Uporabniške nastavitve">
    ...
    {aiOpen && <div className={`${styles.headerPopover} ${styles.aiPopover}`}><strong>Pinart AI asistent</strong>{hasAi ? <><p>Pomaga ti razumeti rezultate, pripraviti osnutke in najti naslednji korak.</p><button type="button" className={styles.aiCta}>Začni pogovor</button></> : <><p>AI asistent je del plačljivega paketa. Brezplačni kalkulator ostaja brezplačen.</p><Link className={styles.aiCta} href={`${base}/kalkulator/paket`}>Nadgradi paket</Link></>}</div>}
